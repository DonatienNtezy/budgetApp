      ...
      // ID = last ID + 1
       ...
       calculateBudget: function() {
          ...
          // Calculate the budget: income - expenses
           ...
           //Expense = 100 and Income = 300, spent is 33.333% = 100/300 = 0.3333 * 100
...
return {
    ...
    now = new Date(); // display the current date
...
document.querySelector(DOM.inputBtn).addEventListener('click', ctrlAddItem);
 ...
 document.addEventListener('keypress', function(event){
     ...
     if (event.keyCode === 13 || event.Show === 13){
   ...
   document.querySelector(DOM.inputType).addEventListener('change', UICtrl.changedType);     
    ...
    input = UICtrl.getInput();
        ...
        if(input.description !== "" && isNaN(input.description) && !isNaN(input.value) && input.value > 0) {
           // 2. Add the itm to the budget controller
